  ...
  <div class="d-flex gap-5 white-space">
    ...
    <div class="mt-3" style="width: 100%">
      ...
      <div class="bg p-4">
          ...
          <tbody class="w-100">
            <tr v-for="dto in userInfo.dtoList" :key="dto" class="w-100">
              ...
              <td><button type="button" class="btn btn-primary" data-bs-toggle="modal"
                  :data-bs-target="`#userDetailModal${dto.mid}`">관리</button></td>
        ...
        <nav aria-label="Page">
          <ul class="pagination">
            <li class="page-item"><a class="page-link" @click="getUserList(userInfo.page - 1)"
                v-if="userInfo.page != 1">Previous</a></li>
            <li :class="userInfo.page == page ? 'page-item active' : 'page-item'" v-for="page in userInfo.pageList"
              :key="page">
              <a class="page-link" @click="getUserList(page)">{{ page }}</a>
            </li>
            <li class="page-item"><a class="page-link" @click="getUserList(userInfo.page + 1)"
                v-if="userInfo.page != userInfo.totalPage">Next</a></li>
          </ul>
        </nav>
      </div>
    </div>
